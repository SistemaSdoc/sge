...
export default function Show({ turma, pauta, periodo: initialPeriodo, instituicao, cursoTutelado }) {
  ...
  useEffect(() => {
    if (periodo !== initialPeriodo) {
      ...
      router.get(window.location.pathname, { periodo }, {
        preserveState: false,
        onSuccess: (page) => {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6">
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-bold">{turma?.nome}
          <p>{turma?.classe} — {turma?.turno}</p>
        </h1>
