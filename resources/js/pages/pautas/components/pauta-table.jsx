...
const formatarNota = (n) => n !== null && n !== undefined ? parseFloat(Number(n).toFixed(1)) : "—"

// ── Badge de situação por disciplina ─────────────────────────────────────

const SITUACAO_CONFIG = {
  aprovado: { label: "Aprovado", className: "bg-green-50 text-green-600 border-green-200" },
  transita_com_deficiencia: { label: "Def.", className: "bg-amber-50 text-amber-600 border-amber-200" },
  recurso_aprovado: { label: "Recurso ✓", className: "bg-blue-50 text-blue-600 border-blue-200" },
  recurso_reprovado: { label: "Reprovado", className: "bg-red-50 text-destructive border-red-200" },
  reprovado: { label: "Reprovado", className: "bg-red-50 text-destructive border-red-200" },
...
function ResultadoBadge({ resultado }) {
  ...
  const config = {
    transita: { label: "Transita", className: "bg-green-50 text-green-600 border-green-200" },
    transita_com_deficiencia: { label: "c/ Deficiência", className: "bg-amber-50 text-amber-600 border-amber-200" },
    recurso: { label: "Recurso", className: "bg-yellow-50 text-yellow-600 border-yellow-200" },
    aprovado_recurso: { label: "Aprovado", className: "bg-green-50 text-green-600 border-green-200" },
    reprovado_recurso: { label: "Reprovado", className: "bg-red-50 text-destructive border-red-200" },
    pendente: { label: "Pendente", className: "bg-muted text-muted-foreground" },
    EEF: { label: "EEF", className: "bg-orange-50 text-orange-600 border-orange-200" },
  }[resultado] ?? { label: resultado, className: "bg-muted text-muted-foreground" }
...
function CelulaNotaDisciplina({ nota, periodo }) {
  ...
  if (!ehPautaEspecial(periodo)) {
    ...
    return (
      <div className="flex flex-col items-center gap-0.5">
        <span className={media < 10 ? "text-destructive font-semibold" : ""}>
    ...
    return (
      <div className="flex flex-col items-center gap-1">
        {nota.media_recurso !== null && (
          <span className={`text-sm font-medium ${nota.media_recurso < 10 ? "text-destructive" : ""}`}>
  ...
  if (ehPautaFinal(periodo)) {
    ...
    return (
      <div className="flex flex-col items-center gap-1">
        ...
        {/* Média final */}
        <span className={`text-sm font-medium ${nota.mf !== null && nota.mf < 10 ? "text-destructive" : ""}`}>
          {formatarNota(nota.mf)}
        </span>

        {/* Nota de recurso — só aparece se existir */}
        {nota.recurso !== null && (
          <span className={`text-xs ${nota.recurso < 10 ? "text-destructive" : "text-yellow-600"}`}>
  ...
  function ResumoFinal({ resumo }) {
    ...
    const items = [
      ...
      { label: "c/ Deficiência", value: resumo.transita_com_deficiencia, className: "text-amber-600" },
      { label: "Recurso", value: resumo.recurso, className: "text-blue-600" },
      { label: "N/Apto", value: resumo.nao_transita, className: "text-destructive" },
  ...
  export function PautaTable({
    ...
    return (
      <Card className="gap-0 w-full max-w-7xl mx-auto">
        <CardHeader className="border-b">
          ...
          <CardAction className="flex items-center gap-3">
            ...
            <Button
              variant="outline"
              disabled
              title="Função de exportar em desenvolvimento"
            >
              Exportar
            </Button>
          </CardAction>
        ...
        {!isEmpty && (
          <CardFooter className="flex-col gap-4 items-start border-t pt-4">
            {/* Resumo — só aparece na pauta final */}
            {ehPautaFinal(periodo) && <ResumoFinal resumo={data?.resumo} />}

            <div className="flex w-full justify-between items-center">
              <span className="text-muted-foreground text-sm">Página 1 de 4</span>
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
