  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <Card className="overflow-hidden pt-0!">
        <div className="relative flex items-end w-full h-56 bg-muted">
          ...
          <div className="relative z-10 flex items-end justify-between w-full p-6">
            <div className="space-y-2 text-white">
              <h1 className="text-2xl font-semibold md:text-3xl">{data?.nome_grupo}</h1>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-full max-w-4">
                <DropdownMenuItem onClick={() => router.push(`/dashboard/pap/grupos/${id}/data-defesa/create`)}>
                ...
                <DropdownMenuItem variant="destructive" onClick={() =>
                  deleteGrupo(id, { onSuccess: () => router.push('/dashboard/pap/grupos') })
                }>
                  Remover grupo
                </DropdownMenuItem>
              </DropdownMenuContent>
        ...
        <CardContent className="grid grid-cols-1 gap-6 py-6 md:grid-cols-4">
          ...
          <div>
            ...
            <p className="font-medium">{data?.turma ?? <Minus size={15} className="text-muted-foreground" />}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Status</p>
            <p className="font-medium">{data?.status ?? <Minus size={15} className="text-muted-foreground" />}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Data de defesa</p>
            <p className="font-medium">{data?.data_defesa ?? "Por definir..."}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="integrantes-grupo" className="w-full">
        <TabsList>
          <TabsTrigger value="integrantes-grupo">Integrantes do grupo</TabsTrigger>
          <TabsTrigger value="integrantes-banca">Integrantes da banca</TabsTrigger>
        </TabsList>
