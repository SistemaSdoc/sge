...
export default function NotificacoesSino() {
  ...
  // ─────────────────────────────────────────────────────────
  // Carregar notificações (JSON via fetch)
  // ─────────────────────────────────────────────────────────
        ...
        <div className="max-h-96 overflow-y-auto">
          {notificacoes.length === 0 ? (
            <p className="p-4 text-center text-sm text-muted-foreground">
              {aCarregar ? 'A carregar…' : 'Sem notificações.'}
            </p>
          ) : (
            notificacoes.map((n) => (
              <button
              ...
              >
                ...
                {/* Parecer do diretor (submissão avaliada — professor) */}
                ...
                {n.tipo === 'nova_submissao' && (
                  <div className="mt-2 space-y-0.5 text-[11px] text-muted-foreground">
                    <p>
                      <strong>{n.professor_nome}</strong>
                    </p>
                    <p>
                      {n.disciplina || 'Todas'}
                      {n.classe && ` • 🎓 ${n.classe}`}
                ...
                {/* Parecer do diretor (justificativa avaliada — professor) */}
