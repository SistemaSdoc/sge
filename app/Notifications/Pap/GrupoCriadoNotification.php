...
class GrupoCriadoNotification extends Notification implements ShouldQueue, ShouldQueueAfterCommit
{
    use Queueable;
    ...
    {
        $alunos = $this->grupoPap->alunos->map(function ($a) {
            return $a->inscricao?->candidato?->nome ?? ($a->aluno?->inscricao?->candidato?->nome ?? 'Sem nome');
        })->toArray();

        return (new MailMessage)
            ->subject('Grupo PAP criado')
            ->view('mail.pap.grupo-criado', [
    ...
    public function toArray(object $notifiable): array
    {
        return [
            'tipo' => 'grupo_criado',
            'titulo' => 'Grupo PAP criado',
            'mensagem' => "Foi criado o grupo \"{$this->grupoPap->nome_grupo}\". Consulte os elementos do grupo.",
