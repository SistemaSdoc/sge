...
test('index lists turmas for the selected academic year', function () {
    Gate::before(fn () => true);
    ...
    $response = $this->actingAs($user)->get(route('turmaGeral', [
        'ano_lectivo_id' => $anoAtual->id,
    ]));

    $response->assertOk();
    $response->assertSee($turmaAtual->nome);
    $response->assertDontSee($turmaAnterior->nome);
});
